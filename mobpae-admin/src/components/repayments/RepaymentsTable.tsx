...
const STATUS_CFG: Record<string, { label: string; color: string; bg: string }> = {
  SCHEDULED: { label: "Scheduled", color: "var(--color-warning)", bg: "var(--color-warning-bg)" },
  PAID:      { label: "Paid",      color: "var(--color-success)", bg: "var(--color-success-bg)" },
  OVERDUE:   { label: "Overdue",   color: "#EF4444", bg: "var(--color-danger-bg)" },
};
      ...
      <table style={{ width: "100%", borderCollapse: "collapse" }}>
        ...
        <tbody>
          {repayments.map(r => {
            ...
            const s = STATUS_CFG[r.status] ?? { label: r.status, color: "var(--color-ink-3)", bg: "var(--color-surface-muted)" };
            const ac = avatarColor(emp.name);
            const rowBg = isSelected ? "var(--color-brand-soft)" : hovered === r.id ? "var(--color-surface-raised)" : "transparent";
            ...
            return (
              ...
              >
                ...
                {/* Principal */}
                <td style={{ padding: "16px 20px", verticalAlign: "middle" }}>
                  <p style={{ fontSize: 13.5, fontWeight: 600, color: "var(--color-ink)", margin: 0, fontVariantNumeric: "tabular-nums" }}>{fmt(r.principalAmount)}</p>
                </td>

                {/* Total */}
                <td style={{ padding: "16px 20px", verticalAlign: "middle" }}>
                  <p style={{ fontSize: 13.5, fontWeight: 600, color: "var(--color-ink)", margin: 0, fontVariantNumeric: "tabular-nums" }}>{fmt(r.totalAmount)}</p>
                </td>
                ...
                {/* Due Date */}
                <td style={{ padding: "16px 20px", verticalAlign: "middle" }}>
                  <p style={{ fontSize: 13, color: r.status === "OVERDUE" ? "#EF4444" : "var(--color-ink-4)", margin: 0, fontWeight: r.status === "OVERDUE" ? 600 : 500 }}>
