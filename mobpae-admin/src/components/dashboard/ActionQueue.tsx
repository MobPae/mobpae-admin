...
export default function ActionQueue({ data, loading = false }: ActionQueueProps) {
  ...
  const items: ActionItem[] = [
    ...
    },
    {
      ...
      countBg: "bg-brand-soft",
      countColor: "text-[#2048EE]",
    },
    {
      label: "Disbursals",
    ...
    },
    {
      ...
      accentBg: "bg-canvas",
      accentColor: "text-ink-3",
      countBg: "bg-surface-muted",
      countColor: "text-ink-3",
    },
    {
      label: "Enquiries",
  ...
  if (loading) {
    return (
      <div className="bg-white border border-edge rounded-2xl shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-edge">
          <div className="h-4 w-28 bg-surface-muted rounded animate-pulse" />
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-3 divide-x divide-y divide-edge">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="p-5">
              <div className="w-8 h-8 rounded-xl bg-surface-muted animate-pulse mb-3" />
              <div className="h-5 w-10 bg-surface-muted rounded animate-pulse mb-1.5" />
  ...
  return (
    <div className="bg-white border border-edge rounded-2xl shadow-sm overflow-hidden">
      <div className="px-6 py-4 border-b border-edge flex items-center justify-between">
        <h2 className="text-[13px] font-semibold text-ink">Action Queue</h2>
        <span className="text-[11px] text-ink-3">Click to navigate</span>
      </div>
      <div className="grid grid-cols-2 sm:grid-cols-3 divide-x divide-y divide-edge">
        {items.map((item) => {
          ...
          return (
            <button
              ...
              className="p-5 text-left hover:bg-canvas transition-colors group"
            >
              ...
              <div className="flex items-baseline gap-2 mb-0.5">
                <span className="text-[22px] font-[750] tracking-tight text-ink leading-none">
                  {item.count}
                </span>
                {hasCount && (
              ...
              <div className="text-[11px] text-ink-3 mt-0.5">{item.description}</div>
            </button>
          );
        })}
      </div>
