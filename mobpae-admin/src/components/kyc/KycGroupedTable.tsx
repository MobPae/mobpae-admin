...
const OVERALL_STATUS: Record<string, { dot: string; text: string; bg: string; label: string }> = {
  ...
  NOT_SUBMITTED: { label: "Not submitted", dot: "bg-[#9CA3AF]", text: "text-[#6B7280]", bg: "bg-[#F3F4F6]" },
};
          ...
          {groups.map(g => {
            ...
            return (
              ...
              >
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2.5 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className="text-[12px] font-[500] text-[#111827] truncate leading-none">{g.employeeName}</p>
                      <p className="text-[11px] text-[#6B7280] font-mono mt-0.5 leading-none">{g.employeeCode}</p>
                    </div>
                  </div>
                </td>
                {/* Employer */}
                <td className="px-4 py-3">
                  <p className="text-[12px] font-[500] text-[#6B7280] truncate leading-none">{g.companyName}</p>
                </td>
                {/* Count cells */}
                <td className="px-4 py-3">
                  <span className="text-[13px] font-[600] text-[#6B7280] tabular-nums">{g.submittedCount}</span>
                </td>
                <td className="px-4 py-3">
                ...
                <td className="px-4 py-3">
                  <span className={`text-[13px] font-[600] tabular-nums ${g.rejectedCount > 0 ? "text-red-500" : "text-[#6B7280]"}`}>
                    {g.rejectedCount}
                  </span>
                </td>
                {/* Overall status */}
                ...
                {/* Doc count chips */}
                <td className="px-4 py-3">
                  <span className="text-[11px] text-[#6B7280] font-[500]">
