    ...
    <div className="px-8 py-6 space-y-5">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 20, marginBottom: 28, marginTop: 24 }}>
        {[
          ...
          { label: "Overdue",          val: counts["OVERDUE"]   ?? 0,  icon: <AlertTriangle size={14} />,   bg: "bg-danger-soft",    color: "text-danger"   },
          { label: "Recovered",        val: counts["PAID"]      ?? 0,  icon: <CheckCircle2 size={14} />,    bg: "bg-brand-soft", color: "text-brand" },
          { label: "Total recovered", val: fmtCurrency(totalRecovered),                                  icon: <CircleDollarSign size={14} />,bg: "bg-surface-muted",  color: "text-ink-3"   },
        ].map(({ label, val, icon, bg, color }) => (
          <div key={label} className="bg-white border border-edge rounded-xl px-4 py-3.5 flex items-center gap-3">
            <div className={`w-7 h-7 rounded-lg ${bg} ${color} flex items-center justify-center flex-shrink-0`}>{icon}</div>
            <div>
              <p className="text-[20px] font-[600] text-ink leading-none tabular-nums">{val}</p>
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          {FILTERS.map(f => (
            ...
            >
              {f.label}
              {f.value === "ALL" && (
                <span className={`text-[11px] font-[700] ${filter === "ALL" ? "text-white/60" : "text-ink-3"}`}>{counts["ALL"] ?? 0}</span>
              )}
            </button>
          ))}
        </div>
      ...
      <div style={{ background: "white", borderRadius: 20, border: "1px solid #E5E7EB", overflow: "hidden" }}>
        {isError ? (
          <div className="px-6 py-14 text-center">
            ...
            <p className="text-[12px] text-ink-3 mt-1">Check your connection and try again.</p>
            <button onClick={() => void refetch()} className="mt-4 h-8 px-4 text-[12px] font-[500] bg-white border border-edge rounded-lg hover:bg-canvas transition-colors text-ink-3">
              Retry
            </button>
          </div>
        ) : isLoading ? (
          ...
          <div className="py-16 text-center">
            <div className="w-10 h-10 rounded-xl bg-surface-muted flex items-center justify-center mb-3 mx-auto">
              <RefreshCcw size={18} className="text-ink-3" />
            </div>
            <p className="text-[13px] font-[500] text-ink-3">No recoveries found</p>
            <p className="text-[12px] text-ink-3 mt-1">Recoveries appear once salary advances are disbursed</p>
          </div>
        ) : (
              ...
              <thead>
                <tr className="border-b border-edge">
                  {["Employee", "Employer", "Principal", "Interest", "Total Recovery", "Recovery Date", "Status", ""].map(h => (
                    <th key={h} className="px-4 py-3 text-left text-[11px] font-[500] text-ink-3 whitespace-nowrap">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y divide-edge-2">
                {rows.map(r => (
                  <tr
                    ...
                    className={`cursor-pointer hover:bg-canvas/60 transition-colors ${selected?.loanApplicationId === r.loanApplicationId ? "bg-brand-soft/30" : ""} ${r.status === "OVERDUE" ? "bg-danger-soft/20" : ""}`}
                  >
                    <td className="px-4 py-3">
                      <p className="font-[500] text-ink">{r.employeeName}</p>
                      <p className="text-[11px] text-ink-3">{r.employeeCode}</p>
                    </td>
                    <td className="px-4 py-3 text-ink-3">{r.companyName}</td>
                    <td className="px-4 py-3 tabular-nums font-[600] text-ink whitespace-nowrap">{fmtCurrency(r.principalAmount)}</td>
                    <td className="px-4 py-3 tabular-nums text-ink-3 whitespace-nowrap">
                      {parseFloat(r.interestAmount) > 0 ? fmtCurrency(r.interestAmount) : <span className="text-ink-3">—</span>}
                    </td>
                    <td className="px-4 py-3 tabular-nums font-[600] text-ink whitespace-nowrap">{fmtCurrency(r.totalRecoveryAmount)}</td>
                    <td className={`px-4 py-3 tabular-nums whitespace-nowrap ${r.status === "OVERDUE" ? "text-danger font-[500]" : "text-ink-3"}`}>
                    ...
                    <td className="px-4 py-3">
                      <span className="flex items-center gap-0.5 text-[12px] font-[500] text-brand whitespace-nowrap">
      ...
      {selected && (
        <>
          ...
          <div className="fixed inset-y-0 right-0 z-40 w-[460px] bg-white border-l border-edge shadow-overlay flex flex-col">
            <div className="px-5 pt-5 pb-4 border-b border-edge">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-[16px] font-[700] text-ink">{selected.employeeName}</p>
                  <p className="text-[12px] text-ink-3 mt-0.5">{selected.employeeCode} · {selected.companyName}</p>
                </div>
                <button onClick={() => setSelected(null)} className="w-7 h-7 flex items-center justify-center rounded-lg border border-edge text-ink-3 hover:text-ink-3">
                  <X size={14} />
                </button>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
              {selected.status === "OVERDUE" && (
                <div className="bg-danger-soft border border-red-100 rounded-xl p-3.5 flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-danger mt-0.5">This recovery is past its scheduled date.</p>
                  </div>
                </div>
              )}

              <div>
                <p className="text-[11px] font-[600] text-ink-3 uppercase tracking-[0.07em] mb-2">Recovery Amounts</p>
                <div className="bg-white border border-edge rounded-xl px-4 py-1">
                  ...
                  <div className="flex items-center justify-between py-3 border-t border-edge mt-1">
                    <span className="text-[12px] font-[600] text-ink">Total recovery</span>
                    <span className="text-[14px] font-[700] text-ink tabular-nums">{fmtCurrency(selected.totalRecoveryAmount)}</span>
                  </div>
                </div>
              </div>

              <div>
                <p className="text-[11px] font-[600] text-ink-3 uppercase tracking-[0.07em] mb-2">Dates</p>
                <div className="bg-white border border-edge rounded-xl px-4 py-1">
              ...
              {selected.raw.remarks && (
                <div>
                  <p className="text-[11px] font-[600] text-ink-3 uppercase tracking-[0.07em] mb-2">Remarks</p>
                  <div className="bg-canvas border border-edge rounded-xl px-4 py-3">
                    <p className="text-[12px] text-ink-3 leading-relaxed">{selected.raw.remarks}</p>
                  </div>
                </div>
              )}
            </div>
