...
const TYPE_CONFIG: Record<NotificationType, { label: string; cls: string }> = {
  ...
  SMS:    { label: "SMS",    cls: "bg-success-bg text-success-dark" },
};
  ...
  return (
    <div className="p-6 max-w-[1100px] mx-auto space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-[16px] font-[700] text-ink leading-none">Notifications</h1>
          <p className="text-[12px] text-ink-4 mt-0.5">Send targeted notifications to employees</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-[420px_1fr] gap-6 items-start">
        ...
        {/* ── SEND FORM ─────────────────────────────────────────── */}
        <div className="bg-white rounded-xl border border-edge shadow-sm p-5 space-y-4">
          <h2 className="text-[13px] font-[600] text-ink">Send notification</h2>

          {/* Employee picker */}
          <div className="relative">
            <label className="block text-[11px] font-[600] text-ink-3 mb-1.5 uppercase tracking-wide">
              Recipient
            </label>
            {selected ? (
              <div className="flex items-center justify-between px-3 py-2.5 border border-brand rounded-lg bg-brand-soft/40">
                <div>
                  <p className="text-[12px] font-[600] text-ink">{selected.name}</p>
                  <p className="text-[11px] text-ink-4">{selected.email} · {selected.employeeCode}</p>
                </div>
                <button
                  type="button"
                ...
                {showDrop && filtered.length > 0 && (
                  <div className="absolute z-20 top-full left-0 right-0 mt-1 bg-white border border-edge rounded-lg shadow-raised overflow-hidden">
                    {filtered.map(emp => (
                      ...
                      >
                        <p className="text-[12px] font-[500] text-ink">{emp.name}</p>
                        <p className="text-[11px] text-ink-4">{emp.email} · {emp.employeeCode}</p>
                        {!emp.userId && (
        ...
        {/* ── NOTIFICATIONS LOG ─────────────────────────────────── */}
        <div className="bg-white rounded-xl border border-edge shadow-sm overflow-hidden">
          <div className="px-5 py-3.5 border-b border-edge flex items-center justify-between">
            <h2 className="text-[13px] font-[600] text-ink">Notification log</h2>
            {notifData && (
              <span className="text-[11px] text-ink-4">{notifData.total} total</span>
            )}
          </div>

          {notifLoading ? (
            <div className="flex items-center justify-center h-48 text-ink-disabled">
              <Loader2 size={18} className="animate-spin" />
            </div>
          ) : notifications.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-48 gap-2 text-ink-disabled">
              <BellOff size={22} />
            ...
            <>
              <div className="divide-y divide-edge-2">
                {notifications.map(n => (
                  <div key={n.id} className="px-5 py-3 hover:bg-surface-raised transition-colors">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2 mb-0.5">
                          ...
                          <span className="text-[12px] font-[600] text-ink truncate">{n.title}</span>
                        </div>
                        <p className="text-[11px] text-ink-3 line-clamp-2">{n.message}</p>
                        <p className="text-[10px] text-ink-disabled mt-1" title={fmtDate(n.createdAt)}>
                ...
                <div className="px-5 py-3 border-t border-edge flex items-center justify-between">
                  ...
                  <div className="flex gap-1.5">
                    <button
                      ...
                      disabled={page === 1}
                      className="w-7 h-7 flex items-center justify-center rounded-md border border-edge text-ink-3 hover:bg-canvas disabled:opacity-40 transition-colors"
                    >
                      <ChevronLeft size={13} />
                    </button>
                    <button
